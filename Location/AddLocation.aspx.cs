...
public partial class Location_AddLocation : System.Web.UI.Page
{
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string location = txtLocationName.Text.ToString();
            ...
            if (BtnSave.Text.Equals("Update"))
            {
                ...
                string query = "UPDATE [Zipcode] SET [Location]='"+ location + "',[zipcode]='"+ zipcode + "',[State]='"+ state + "',[District]='"+ city+ "',[IsActive] = "+ IsActive + ",[ModifiedOn]='" + dt.ToString() + "',[ModifiedBy]="+ userId + " Where  Id = " + id;
                int v1 = dbc.ExecuteQuery(query);
                if (v1 > 0)
                {
                    sweetMessage("", "Location Updated Successfully", "success");
            ...
            else
            {
                ...
                if (VAL > 0)
                {
                    sweetMessage("", "Location Added Successfully", "success");
                    Response.Redirect("Location.aspx", true);
                }
                else
                {
